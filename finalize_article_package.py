...
def _captions(headline: str, image_count: int) -> list[str]:
    """Human-facing editorial captions, not technical camera-angle labels."""
    subject = _subject_label(headline)
    templates = [
        f'{subject}: строгий и узнаваемый образ модели, в котором сочетаются фирменная передняя часть, современная светотехника и классические пропорции седана.',
        f'{subject}: задняя часть автомобиля подчёркивает спокойный премиальный характер модели и узнаваемую графику фонарей.',
        f'{subject}: вытянутый силуэт, длинная колёсная база и сбалансированные пропорции хорошо показывают характер автомобиля сбоку.',
        f'{subject}: салон ориентирован на водителя и повседневный комфорт — передняя панель, органы управления и посадка собраны вокруг удобства в дороге.',
        f'{subject} в обычной эксплуатации: такой кадр помогает представить реальные габариты автомобиля и то, как он выглядит в повседневной дорожной среде.',
    ]
