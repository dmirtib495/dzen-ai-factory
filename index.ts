...
function menu() {
  return {
    inline_keyboard: [
      ...
      [
        { text: "🚗 Создать 3 статьи", callback_data: "generate" },
...
async function analytics(env: Env) {
  ...
  return (
    ...
    r.results
      .map((x: any) => {
        const er = (
          ((x.likes + x.comments * 2 + x.shares * 3) / Math.max(x.views, 1)) *
          100
        ).toFixed(2);
...
async function strategy(env: Env) {
  ...
  if (!r) {
    return "🧠 Стратегия ещё не рассчитана. Она обновится после появления статистики.";
  }
...
async function articleText(env: Env, id: number) {
  return env.DB.prepare(`
    SELECT headline, category, article_markdown, source_urls_json,
           fact_check_json, status
    FROM articles
    WHERE id = ?
  `)
    .bind(id)
...
async function handleCallback(
  ...
  if (data === "generate") {
    return send(
      env,
      chat,
      "🚗 Команда принята. GitHub Actions запустит генерацию; отдельный Worker не держит Python-процесс постоянно.",
      menu()
    );
  }
  ...
  if (m) {
    ...
    return send(
      env,
      chat,
      `✅ Материал #${id} одобрен. Публикация остаётся отдельным официальным шагом Дзена.`,
      menu()
    );
  }
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/health") {
      return json({ ok: true, service: "dzen-auto-control" });
    }
